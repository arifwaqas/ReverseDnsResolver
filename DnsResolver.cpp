...
void helper(std::string& ipin, std::string& ipdest)
{
	...
	else
	{
		...
		dest.sin_addr.s_addr = inet_addr(ipdest.c_str());
	}
	...
	dns->cd = 0;
	dns->rcode = 0;
	dns->q_count = htons(1); // count of queries
	dns->ans_count = 0;
	dns->auth_count = 0;
	dns->add_count = 0;
	...
	// preprocess the ip we need to query
	auto hostip = preprocess_ipv4(ipin);
	const char* hostipc{ hostip.c_str() };
	dns_format(qname, hostipc);

	//ChangetoDnsNameFormat(qname, host);
	qinfo = (struct QUESTION*)&buf[sizeof(struct DNS_HEADER) + (strlen((const char*)qname) + 1)]; //fill it

	qinfo->qtype = htons(0x000C); //we are requesting the ipv4 address
	qinfo->qclass = htons(0x0001); //its internet (lol)
